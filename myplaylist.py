...
cnxn = pymysql.connect(host='localhost', port=3306, user='root', password='1234',
                       db='myplaylistdb', charset='utf8')
...
def edit_playlist():  # *  3번
    ...
    while True:
        ...
        select = input('선택한 번호의 노래 제거 | 뒤로가기(b) | 플레이리스트삭제(r) : ')
        if select == 'b':
            menu()
            break
        elif select == 'r':
            ...
            cursor.execute(
                "DELETE FROM track where user_id='%s' and track_id='%s'" % (Uid, row[int(select)-1][3]))
            cnxn.commit()
            print('%s - %s (이)가 제거되었습니다' %
                  (row[int(select)-1][0], row[int(select)-1][1]))
            break
...
def show_playlist():  # *  4번
    print('# 플레이리스트 목록')
    cursor.execute(
        "select a.title, a.user_id, round(avg(b.reco_num),1) FROM playlist a LEFT JOIN recommend b ON a.playlist_id = b.playlist_id group by a.playlist_id, a.title, a.user_id order by title")
    row = cursor.fetchall()
    ...
    if len(row2) > 0:
        for i in range(len(row2)):
            print(' ㄴ %-6s : %s' % (row2[i][0], row2[i][1]))

    choice = input('댓글달기 [1] | 평점매기기[2] | 즐겨찾기 추가[3] | 뒤로가기 [b] : ')
    if choice == 'b':
        menu()
    elif int(choice) == 2:
        add_recommend(Pid)
...
def show_bookmark():  # * 5번
    print('#즐겨찾기목록')
    cursor.execute(
        "SELECT distinct a.user_id, a.title FROM playlist a INNER JOIN bookmark b ON a.playlist_id = b.playlist_id order by title")
    row = cursor.fetchall()

    print(tabulate(row, headers=['작성자', '제목'],
          tablefmt='pretty', stralign='left', showindex=True))


def add_recommend(Pid):
...
def show_myplaylist(Uid):
    ...
    print('---------------------------------------')
    cursor.execute(
        "SELECT DISTINCT `track_name`, `artist_name`, `genres`, `track_id` FROM (SELECT a.track_name, a.artist_name, a.track_id, GROUP_CONCAT(b.genre) AS genres FROM song a INNER JOIN genres b ON a.track_id = b.track_id GROUP BY a.track_id) c where track_id IN( SELECT track_id from track where user_id = '%s')" % (Uid))
    row = cursor.fetchall()
...
def menu():
    ...
    print('3.내 플레이리스트 수정\t4.플레이리스트 목록')
    print('5.즐겨찾기 목록')
    print('9.종료')
    return int(input('입력: '))
