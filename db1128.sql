...
SELECT DISTINCT `track_name`, `artist_name`, `genres`, `track_id` FROM 
(SELECT a.track_name, a.artist_name, a.track_id, GROUP_CONCAT(b.genre) AS genres FROM song a INNER JOIN genres b ON a.track_id = b.track_id GROUP BY a.track_id) c 
where playlist_id IN( SELECT track_id from track where playlist_id = '%s');

SELECT DISTINCT `track_name`, `artist_name`, `genres`, `track_id` FROM 
(SELECT a.track_name, a.artist_name, a.track_id, GROUP_CONCAT(b.genre) AS genres FROM song a INNER JOIN genres b ON a.track_id = b.track_id GROUP BY a.track_id) c INNER JOIN playlist d ON 
-- 노래 검색
